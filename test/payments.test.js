...
describe('Payments', function () {
  ...
  it('should pass on an execution condition fulfillment', async function () {
    ...
    assert.equal(result.fulfillment, 'HS8e5Ew02XKAglyus2dh2Ohabuqmy3HDM8EXMLz22ok')
  })

  it('should pass on even an invalid fulfillment', async function () {
    // We only have one shot at fulfilling the incoming transfer and getting
    // paid back. There isn't really a reason not to at least try passing on
    // the fulfillment we got, even if it seems invalid.
    sinon.stub(this.mockPlugin2Wrapped, 'sendTransfer')
    .resolves({
      fulfillment: 'invalid',
      data: Buffer.alloc(0)
    ...
    assert.equal(result.fulfillment, 'invalid')
  })

  it('passes on the executionCondition and reduces the expiry date by one second', async function () {
  ...
  it('uses best rate by default', async function () {
    const sendSpy = sinon.stub(this.mockPlugin2Wrapped, 'sendTransfer')
      .resolves({
        fulfillment: 'HS8e5Ew02XKAglyus2dh2Ohabuqmy3HDM8EXMLz22ok',
        data: Buffer.alloc(0)
  ...
  // TODO ENLIGHTEN What is this functionality used for?
  ...
  describe('rejection', function () {
    it('relays an interledger rejection', async function () {
      const rejectStub = sinon.stub(this.mockPlugin2Wrapped, 'sendTransfer')
        .rejects({
          name: 'InterledgerRejectionError',
          ilpRejection: packet.serializeIlpRejection({
            code: '123',
            message: 'Error 1',
