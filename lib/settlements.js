...
Settlements.prototype.settlementIsPrepared = function (settlement) {
  let isPrepared = true
  for (let sourceTransfer of settlement.source_transfers) {
    if (sourceTransferIsPrepared(sourceTransfer)) {
      if (this.sourceSubscriptions.get(sourceTransfer.id)) {
        this.sourceSubscriptions.remove(sourceTransfer.id)
      }
    } else {
      this.sourceSubscriptions.put(sourceTransfer.id, settlement)
      isPrepared = false
    }
  }
  return isPrepared
...
Settlements.prototype.validateExpiry = function (settlement) {
...
Settlements.prototype.amountFinder = function (ledger, creditOrDebit) {
 ...
 */
Settlements.prototype.calculateAmountEquivalent = function * (opts) {
...
Settlements.prototype.validateRate = function * (settlement) {
...
Settlements.prototype.validateFee = function * (settlement) {
  // Determine which ledger's asset we will convert all
  // of the others to
  let convertToLedger
  if (settlement.source_transfers.length === 1) {
...
Settlements.prototype.addAuthorizationToTransfers = function (transfers) {
...
Settlements.prototype.submitTransfers = function * (transfers, correspondingSourceTransfers) {
  for (let transfer of transfers) {
...
Settlements.prototype.validate = function * (settlement) {
  ...
  this.validateExpiry(settlement)
  yield this.validateRate(settlement)
  yield this.validateFee(settlement)
  validateExecutionConditions(settlement)
  yield validateExecutionConditionPublicKey(settlement)

  return this.settlementIsPrepared(settlement)
}

Settlements.prototype.settle = function * (settlement) {
  if (settlement.destination_fee_transfers) {
    log.debug('submitting destination fee transfers')
    this.addAuthorizationToTransfers(settlement.destination_fee_transfers)
    yield this.submitTransfers(settlement.destination_fee_transfers)
  }

  log.debug('submitting destination transfers')
  this.addAuthorizationToTransfers(settlement.destination_transfers)
  yield this.submitTransfers(settlement.destination_transfers,
...
Settlements.prototype.updateTransfer = function * (updatedTransfer) {
  // Maybe its a source transfer:
  let settlement = this.sourceSubscriptions.get(updatedTransfer.id)
  if (settlement) {
    yield this.updateSourceTransfer(updatedTransfer, settlement)
    return
  }
...
// settlement. When all of the settlement's source transfers are "prepared",
// authorized/submit the payment.
Settlements.prototype.updateSourceTransfer = function * (updatedTransfer, settlement) {
  let target_id = updatedTransfer.id
  let source_transfers = settlement.source_transfers
  ...
  if (updatedTransfer.state === 'prepared') {
    ...
    if (!this.sourceSubscriptions.hasSettlement(settlement)) {
      yield this.settle(settlement)
    }
  }
}

Settlements.prototype.updateDestinationTransfer = function * (updatedTransfer, sourceTransfers) {
