...
module.exports = function * (config, payment) {
  let tester = new TransferTester(config, payment)
...
TransferTester.prototype.getExpiry = function (transfer) {
  ...
  let expiries = _.uniq(_.values(_.pick(
...
TransferTester.prototype.validateTransferNotExpired = function (transfer) {
  let expires_at = this.getExpiry(transfer)
...
TransferTester.prototype.validateMaxHoldTime = function * () {
  for (let transfer of this.destination_transfers) {
    let expires_at = this.getExpiry(transfer)
    if (expires_at) {
...
TransferTester.prototype.validateMinExecutionWindow = function * () {
  let minExecutionWindow = this.minExecutionWindow
  let minMessageWindow = this.minMessageWindow
...
TransferTester.prototype.loadCaseExpiries = function * () {
  for (let transfer of this.transfers) {
    ...
    let caseIDs = getTransferCases(transfer)
...
TransferTester.prototype.loadCaseExpiry = function * (caseID) {
  ...
  let expiry = caseRes.body.expires_at
  if (expiry) {
